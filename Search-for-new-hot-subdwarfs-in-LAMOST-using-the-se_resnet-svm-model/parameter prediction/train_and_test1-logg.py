...
def test(model,lossfunction, epoch):
    ...
    with torch.no_grad():
        ...
        print("mae:",mae)
        if mae < bestmae :
            torch.save(model.state_dict(), f'/home/admin1/data1/czd/pred/modellogg/trained_model_8-700-1e-2-logg_epoch{epoch}.pth')
            print("All Outputs for Epoch {}: {}".format(epoch, all_outputs))
            print(len(all_outputs))
            #if all_outputs[0] > 10000:
            ...
            plt.xlim(4.5, 7)
            plt.ylim(4.5, 7)
            plt.xticks([4.5, 5.0, 5.5, 6.0,6.5,7.0])
            plt.yticks([4.5, 5.0, 5.5, 6.0,6.5,7.0])
            plt.tick_params(labelsize=25)
            plt.xlabel('log g (Se-ResNet)', fontsize=25)
            plt.ylabel('log g (Culpan2022)', fontsize=25)
            plt.scatter(all_outputs, all_labels, c='none', marker='o', edgecolors='teal')
            plt.tight_layout()
            plt.savefig(f"/home/admin1/data1/czd/pred/testdrawlogg/logg{epoch}", dpi=300)
            bestmae = mae
        # 计算总体 MAE
...
#################################数据集划分####################
    ...
    writer = SummaryWriter("/home/admin1/data1/czd/pred/logslogg/8-700-1e-2-logg/")
    ...
    writer.close()
    # 保存训练好的模型
    #torch.save(model.state_dict(), '/home/admin1/data1/czd/pred/model2/trained_model4-1000-1e-3-logg.pth')
